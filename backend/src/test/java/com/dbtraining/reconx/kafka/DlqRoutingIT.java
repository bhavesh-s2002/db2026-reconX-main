...
/**
 * ============================================================================
 * TICKET-ADV144 — DlqRoutingIT (Testcontainers DLQ Failure-Path Integration Test)
 * ============================================================================
 */
@SpringBootTest
@Testcontainers
class DlqRoutingIT {
    ...
    @MockBean AuditLogRepository auditRepo;

    @Test
    void failingConsumerRoutesToDlq() {
        Mockito.doThrow(new RuntimeException("boom"))
                .when(auditRepo).save(Mockito.any());

        TradeEvent event = TradeEvent.created(
                "TRD-DLQ-1",
                JsonNodeFactory.instance.objectNode().put("price", 100).toString()
        );
    ...
    private boolean dlqHas(String tradeRef) {
        ...
        try (var consumer = new org.apache.kafka.clients.consumer.KafkaConsumer<String, TradeEvent>(p)) {
            ...
            for (ConsumerRecord<String, TradeEvent> r : records) {
                if (r.value() != null && tradeRef.equals(r.value().tradeRef())) return true;
            }
        }
        return false;
    }
